...
updatePlots = function() {
	...
	var saveColors = state.colors
	if (state.gColors.length > 0) {
		var dType = $('#seq-display').val()
		if (dType=='Interpolated'){
			state.colors = state.gColors
		} else if (dType=='Both'){
			state.colors = state.colors.concat(state.gColors)
		}
	}
	...
	state.colors = saveColors
}


hex2tip = function(hex) {
	var c = chroma(hex)
	var lch = c.lab()	//just for now
...
function displayColorspace() {
	...
	var abRange = Math.max((plotSpec.aMax-plotSpec.aMin),(plotSpec.bMax-plotSpec.bMin))
	var lRange = plotSpec.lMax-plotSpec.lMin
	var lScale = (plotSpec.abSize-plotSpec.lTop)/lRange
	var abScale = plotSpec.abSize/abRange
	var abCenter = abScale*Math.max(Math.abs(plotSpec.aMin),Math.abs(plotSpec.bMin))
	...
	function toPixels (lab) {
		...
		cAB[0] = abCenter + abScale*lab[1]
		cAB[1] = (abSize-abCenter) - abScale*lab[2]	//stupid upside down Y
...
//create the svg frames. Will eventually add axes, etc. here
	...
	svg_ab.append("line").attr("x1", 0).attr("y1", abSize-abCenter)
		.attr("x2",abSize).attr("y2",abSize-abCenter)
		.style({stroke: "lightgray", "stroke-width": "1px"})
	svg_ab.append("line").attr("x1", abCenter).attr("y1",0)
		.attr("x2", abCenter).attr("y2",abSize)
		.style({stroke: "lightgray", "stroke-width": "1px"})
