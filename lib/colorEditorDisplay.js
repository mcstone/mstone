...
//create the svg frames. Will eventually add axes, etc. here
	var svg_ab = d3.select("#abPlot")
		...
		.style("background-color", plotSpec.divFill)
...
function displayAllPalettes() {
	...
	d3.select('#pSpace').style("background-color", plotSpec.divFill)
...
//if these controls are active, then selection sets the background
//if useAsPreview == true, then bColors should be set to be the actual colors put into the background.
var flcControls = {active: false, useAsPreview: false, bColors: null, lastPreview: "#ffffff"}
function displayExamples(stats) {  
	...
	var gap = 10
	var arrayBox
	var pType = $('#select-pType').val().split('.')
	if(pType[0]=='formatting'){
		pSize = 13
		pW = (pSize+2)*10
		pH = (pSize+2)*6
		$('#fl_controls').show()
		flcControls.active = $('#flc_background').prop('checked')
		flcControls.useAsPreview = $('#flc_preview').prop('checked')
		if (flcControls.useAsPreview == true){
		//convention is that preview palettes have a name that is foo_preview
			var name = $("#select-palette").val()
			var base = name.split('_')
			var pIndex = findPalette(base)
			if (pIndex == -1) {
				console.log(base, "not found")
				flcControls.useAsPreview = false
			}
			else {flcControls.bColors = state.palettes[pIndex].eColors}
		}
	} else {
		$('#fl_controls').hide()
	}
	
	d3.select('#pExamples').style('background-color',plotSpec.divFill)
	if (state.colors.length > 28) {
		...
		arrayBox = d3.select('#pExamples').append('div').append('svg')
			...
			.attr("x", 10)
			.attr("y", 10)
		drawArray(arrayBox, pSize, pSize,2, createRectMark)
		if (flcControls.useAsPreview == true){
			arrayBox = d3.select('#pExamples').append('div').append('svg')
			.attr("width",pW)
			.attr("height",pH)
			.attr("x", 10)
			.attr("y", pH+10)
			var temp = state.colors
			state.colors = flcControls.bColors
			drawArray(arrayBox, pSize, pSize,2, createRectMark)
			state.colors = temp
		}
	}
...
myOnClick = function(svgPt) {
	...
	if (c == state.selectedPt) { //just deselect the point
		...
		//here we're going to play with the background color to demo the light formatting
		if (flcControls.active == true) {
			var bColor = state.pColor.hex()
			if (flcControls.useAsPreview==true){
				bColor = flcControls.bColors[cIndex].color.hex()
				flcControls.lastPreview = bColor
			}
