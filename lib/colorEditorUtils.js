...
var T10Mod2 = ['#3185b9', '#fcb025',  '#bf373d', '#679445', '#c9b42f', '#15adac', '#a0608b', '#C7C7C7', '#b28654','#e6949e']
...
function parseHexString(hString) {
	...
	for (var i = 0; i<hexValues.length; i++) { //now trim each item.
	...
	return {pName: name, hex: hexValues}
	}
}
...
function parseXML(xmlString) {
	var doc = jQuery.parseXML(xmlString)
	var colors = $(doc).find("color")	//array of <color>#vvvvvv</color>
	if (colors.length == 0) { 
		console.log("xml error")
		console.log(xmlString)
		return {pName: null, hex: null}
	}
	else {
		var pName = $(doc).find("color-palette").attr("name")  //no problem if this is null
		hexValues = []
		$(colors).each(function () {
			var c = $(this).text().trim()
			hexValues[hexValues.length] = c
		})
		return {pName: pName, hex: hexValues}
	}
} 
		
function initPalettes(state) {
	var pNames = ["Tableau 10","Tableau 20","Tableau Light","Colorblind","Vizable","Cristy All","New Palette", "T10 Mod2"]
	state.palettes = []
