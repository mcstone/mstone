...
function colorItem(color, selected, palette, cType, name, notes) {
	this.color = color
	this.selected = selected
	this.palette = palette
	this.cType = cType	//used to tell a regular from interpolated color
	this.name = name
	this.notes = notes
 }
...
function copyColors(cArray, pName) { //deep copy of an array of color items
	...
	for (var i=0; i<cArray.length; i++) {
		var c = cArray[i]
		colors[i] = new colorItem(chroma(c.color.hex()), c.selected, c.palette, c.cType, c.name)
	}
	return colors
}
