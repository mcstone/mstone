...
function initPalettes(state) {
	var pNames = ["New Palette","Tableau 10","Tableau 20","Tableau Light","Colorblind","Vizable", "T10 Mod2", "Cristy All"]
	state.palettes = []
...
function parseXML(xmlString) {
	...
	$(palettes).each(function () {
			...
			pVals[pVals.length] = {pName: pName, hex: hexValues}
		})
	return pVals
}
...
function paletteToFormattingXML(colors, pType) {
	...
	var numStrings =['first','second','third','fourth','fifth', 'sixth', 'seventh', 'eighth', 'nineth']
	var xml = ""
	var xmlHead = ''
	var cLen = 3
	var nCols = 2
...
function createPalette(hex,pName) {
	...
	for (var i=0; i< hex.length; i++) {
		colors[i] = new colorItem(chroma(hex[i]),false,pName,i.toString())
	}
